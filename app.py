...
def main():
    ...
    st.session_state.chat_type = st.sidebar.selectbox("Select Chat Type", ["Text", "Image", "Pdf"])
    ...
    if st.session_state.chat_type == "Text":
    ...
    elif st.session_state.chat_type == "Image":
        uploaded_image = st.sidebar.file_uploader("or upload an image file", type = ["jpg", "jpeg", "png"])
        if uploaded_image:
            print("Pritni nidn")
            img = Image.open(uploaded_image)
            st.sidebar.image(img, caption="Your Image", width=300)
            uploaded_image.seek(0)

        if send_button or st.session_state.send_input and st.session_state.user_question != "" and uploaded_image:
            ...
            chat_history.add_ai_message("This is a Image Chat type, so you have to give me the image or else you can chat in the Text Chat type.")


    elif st.session_state.chat_type == "Pdf":
        ...
        if send_button or st.session_state.send_input and st.session_state.user_question != "" and uploaded_pdf:
                llm_response = llm_chain.run(st.session_state.user_question)
                st.session_state.user_question = ""
        elif st.session_state.user_question != "":
            chat_history.add_user_message(st.session_state.user_question)
            chat_history.add_ai_message("This is a PDF Chat type, so you have to give me the PDF first or else you can chat in the Text Chat type.")
    ...
    save_chat_history()
    manage_chat_sessions_folder(max_files=10)
